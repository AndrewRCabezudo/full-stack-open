...
describe('when there is initially one user in db', () => {
  beforeEach(async () => {
    ...
    const user = new User({ username: 'root', name: 'Brian Shima', passwordHash })
  ...
  test('password less than 3 characters', async () => {
    ...
    expect(result.body.error).toContain('usernames and passwords must be longer than 3 characters')
    const  usersAtEnd = await helper.usersInDb()
    expect(usersAtEnd).toEqual(usersAtStart)
  })
